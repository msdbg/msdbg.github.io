...
When we pivot execution to this shadow stack, the EDR analyzing the thread sees a chain of return addresses belonging to legitimate modules.
...
If we allocate `PAGE_EXECUTE_READWRITE` (RWX) memory, we instantly burn OPSEC. I've implemented several OPSEC improvements:
...
This can still be chained with TEB patching, module stomping, or hardware breakpoints.

EDRs utilizing **ETW-TI** or periodic memory scanning (e.g., `Get-InjectedThread`) or other heuristics will flag any thread where the RIP points to unbacked executable memory, regardless of how perfect the call stack looks.
...
- **ETW-TI**: Event Tracing for Windows - Threat Intelligence, an ETW provider used by EDRs to monitor thread context changes and other suspicious activities.
...
Once we inspect the process memory, the payload is mapped at `0x1fd11d60000`. By allocating as RW, writing the payload, and using `VirtualProtect` to flip to RX, we map the shellcode into private committed memory while avoiding the highly scrutinized RWX memory indicator.
